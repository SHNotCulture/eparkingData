...
public class CustomizeSqlProvider {
    ...
    public String selectPresentCar(String parkId,String andSql){
        String sql = "SELECT * from t_park_in_out_"+parkId+" WHERE id in (SELECT a.id from (select id,MAX(in_time) from t_park_in_out_"+parkId+" " +
                "where in_time is not null and out_time is NULL and in_car_plate!='未识别'  GROUP BY in_car_plate)a) "+andSql;
        return  sql.toString();
    }
